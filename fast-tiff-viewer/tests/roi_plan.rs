...
fn resident(w: u32, h: u32, off: [f32; 2], scale: [f32; 2], max_axis: u32, bpt: usize) -> Roi {
    plan(w, h, off, scale, max_axis, bpt).resident
}
...
fn a_small_pan_is_served_by_the_window_already_resident() {
    ...
    let before = plan(w, h, [0.5, 0.4], scale, 32_768, RGB8).resident;
    ...
    let after = plan(w, h, [0.5 + scale[0] * 0.25, 0.4], scale, 32_768, RGB8);
...
fn a_large_pan_needs_a_new_window() {
    let (w, h) = ANDROMEDA;
    let scale = [2000.0 / w as f32, 1200.0 / h as f32];
    let before = plan(w, h, [0.1, 0.4], scale, 32_768, RGB8).resident;
    let after = plan(w, h, [0.8, 0.4], scale, 32_768, RGB8);
    assert!(
...
fn zooming_in_asks_for_a_finer_window_than_the_overview() {
    let (w, h) = ANDROMEDA;
    let overview = plan(w, h, WHOLE.0, WHOLE.1, 32_768, RGB8).resident;
    let zoomed = plan(w, h, [0.5, 0.4], [2000.0 / w as f32, 1200.0 / h as f32], 32_768, RGB8);
