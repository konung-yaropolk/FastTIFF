...
fn stacked_the_pointer_side_decides_not_the_grab() {
    ...
    assert_eq!(fresh(0.0, 0.0, 50.0, false), Some(true), "grabbed min, dragging right -> max");
    assert_eq!(fresh(0.0, 0.0, 50.0, true), Some(true));
    // And to the left, min — so a stack at the top of the track is no worse.
    assert_eq!(fresh(1000.0, 1000.0, 900.0, true), Some(false), "grabbed max, dragging left -> min");
...
fn a_committed_drag_keeps_its_handle_so_it_pushes() {
    ...
    assert_eq!(dragged_handle(500.0, 500.0, 600.0, Some(false), false), Some(false));
    // Mirror: max driven leftwards into min keeps pushing min.
    assert_eq!(dragged_handle(500.0, 500.0, 400.0, Some(true), true), Some(true));
    ...
    assert_eq!(dragged_handle(500.0, 500.0, 600.0, Some(false), true), Some(false));
}

#[test]
fn the_choice_survives_the_pair_separating_again() {
    assert_eq!(dragged_handle(500.0, 700.0, 800.0, Some(true), false), Some(true));
    assert_eq!(dragged_handle(300.0, 500.0, 200.0, Some(false), true), Some(false));
}
...
fn stacked_with_no_direction_yet_commits_to_nothing() {
    ...
    assert_eq!(dragged_handle(500.0, 500.0, 500.0, Some(true), false), Some(true));
    assert_eq!(dragged_handle(500.0, 500.0, 500.0, Some(false), true), Some(false));
}
...
fn a_stack_at_either_edge_can_always_be_separated() {
    ...
    for at in [lo, hi, 500.0] {
        assert_eq!(fresh(at, at, at + 10.0, false), Some(true), "right from a stack at {at}");
        assert_eq!(fresh(at, at, at - 10.0, true), Some(false), "left from a stack at {at}");
    }
}
