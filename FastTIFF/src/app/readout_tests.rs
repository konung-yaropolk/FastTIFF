...
fn a_calibrated_frame_reports_its_real_size() {
    assert_eq!(
        physical_size(Some(0.325), Some(0.325), Some("um"), 600, 400).as_deref(),
        Some("195 × 130 um")
    );
}
...
fn anisotropic_pixels_are_not_collapsed() {
    ...
    assert_eq!(
        physical_size(Some(0.2), Some(0.5), Some("um"), 1000, 1000).as_deref(),
        Some("200 × 500 um")
    );
}
...
fn an_uncalibrated_or_unlabelled_frame_reports_nothing() {
    ...
    assert_eq!(
        physical_size(Some(0.3), Some(0.3), Some(""), 100, 100),
        None
    );
